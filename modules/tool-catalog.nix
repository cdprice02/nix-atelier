...
# "bash").
{
  entries = [
    ...
    {
      key = "nixfmt-rfc-style";
      # Confirmed directly, not assumed: nixpkgs's `nixfmt-rfc-style`
      # attribute is a real package, but its own `pname` is always literally
      # "nixfmt" on both pins this repo uses -- the attribute name and the
      # realized identity the drift-checker resolves via `p.pname or p.name`
      # are different things here. `nixfmt-rfc-style` itself never appears
      # as a realized name (first attempt used it and the drift check
      # correctly rejected it as never-installed).
      matches = [ "nixfmt" ];
      category = "Editor";
      description = "The official RFC 166 Nix formatter. This repo's `nix fmt` runs it via treefmt (see treefmt.nix), alongside statix/deadnix/mdformat; also on PATH here directly for Claude Code's format-on-edit hook.";
      link = "[github.com/NixOS/nixfmt](https://github.com/NixOS/nixfmt)";
    }
